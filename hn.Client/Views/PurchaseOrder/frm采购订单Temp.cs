...
namespace hn.Client
{
    public partial class FrmPurchaseTemp : FrmBase
    {
        #region ■------------------ 字段相关
        ...
        public FrmPurchaseTemp(int status = 3)
        {
            InitializeComponent();
        ...
        private void gridView请购计划列表_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            if (e.FocusedRowHandle > -1)
            {
                ...
                gridControl请购计划明细.DataSource = _service.GetPurchasePlanEntryList(fid, null,true);
                this.Cursor = Cursors.Default;
            }
        }
        ...
        private void searchControl1_Properties_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                ...
                if (frm.ShowDialog() == DialogResult.OK)
                {
                    ...
                    txt销区.Text = frm.SelectName;
                    txt销区.Tag = frm.SelectID;
                }
            }
            catch (Exception ex)
            {
