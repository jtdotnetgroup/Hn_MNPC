...
; (function ($) {
    ...
    var createDomId = function() {
        ...
        return "bx-" + (s4() + s4() + "-" + s4() + "-" + s4() + "-" + s4() + "-" + s4() + s4() + s4());
    };
    ...
    function loadingInit(target, options) {
        var $mask = $('<div class="bx-qqmsg-mask"></div>'); //遮罩层
        var $loading = $('<div class="bx-qqmsg-layer_wrap"/>'); //消息
        ...
        if (options.type && options.type != 'loading' ) {
            $loading.appendTo(target).attr("id", layerId + '_loading').empty()
                .append('<span class="bx-qqmsg-layer"  id="mode_tips_v2"><span class="' + iconCls + '"></span>' +
                    options.msg + '<span class="gtl_end"></span></span>').css('z-index', options.zIndex + 1);
        } else {
            $loading.appendTo(target).attr("id", layerId + '_loading').empty()
            .append('<span class="bx-qqmsg-layer"  id="mode_tips_v2"><span class="gtl_ico_clear"></span><span class="' + iconCls + '"></span>' +
                options.msg + '<span class="gtl_end"></span></span>').css('z-index', options.zIndex + 1);
        }
    ...
    $.fn.hLoading.hide = function(jq,fn) {
        return jq.each(function() {
            var wrap = $(this);
            $("div.bx-qqmsg-mask", wrap).fadeOut(function() {
                $(this).remove();
            });
            $("div.bx-qqmsg-layer_wrap", wrap).fadeOut(function() {
                $(this).remove();
            });
