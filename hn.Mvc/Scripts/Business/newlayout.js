...
$(function () {
    if (menus) {
        initNav();
        tabClose();
        //addTab('欢迎使用', 'sys/Main.aspx', 'icon-house_star'); //添加默认TAB
    } else {
    ...
    init: function () {
        ...
        $("#wnav").tree({
            ...
            onClick: function (node) {
                if (node.attributes.url.indexOf('Flow.aspx') == 0) {
                    //alert(node.attributes.url);
                    //window.open(node.attributes.url);
                    window.open(node.attributes.url, 'newwindow', 'height=800,   width=1280,   top=0,   left=0,   toolbar=no,   menubar=no,   scrollbars=no,   resizable=no,location=n   o,   status=no');
                }
                else {
...
function createFrame(url) {
    var s = '<iframe  frameborder="0" id="iframeMain"  style="width:100%;height:100%; padding:2px;"  src="' + url + '" ></iframe>';
    return s;
}
    ...
    top.$.hDialog({
        width: 330, height: 240, title: '修改密码', iconCls: 'icon-key', content: str_editpass, submit: function () {
            if ($('#uiform').form('validate')) {
                $.ajaxjson('/User/Password2', "action=editpass2&old=" + $('#txtOldPassword').val() + "&new=" + $('#txtNewPassword').val(), function (d) {
                    if (d.errCode == 0) {
                        alert('密码修改成功！请重新登录。');
                        location.href = '/Login/Quit';
                    } else
