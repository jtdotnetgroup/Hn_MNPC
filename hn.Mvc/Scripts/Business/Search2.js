...
var search = {
        ...
        var sDialog = top.$.hDialog({
            ...
            onLoad: function () {
                ...
                if (search.toSqlWhere(_filter) == '')
                    top.$('#' + gridid).datagrid('reload', { filter: '' });
                else
                    top.$('#' + gridid).datagrid('reload', { filter: JSON.stringify(_filter) });
    ...
    initFields: function (sel) {
        var columns = top.$('#' + gridid).datagrid('options').columns[0];
